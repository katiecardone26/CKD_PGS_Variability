...
# make merge list
touch merge_lists/filtered/all_chr.merge_list.txt
for chr in $(seq 1 23)
do
echo "input/PMBB_v3_extract/chr_separated_files/PMBB-Release-2024-3.0_genetic_imputed.ckd_pgs_weights_vars.${chr}" >> merge_lists/filtered/all_chr.merge_list.txt
done
sed -i 's/23/X/g' merge_lists/filtered/all_chr.merge_list.txt
# your code here
plink2 --pmerge-list merge_lists/filtered/all_chr.merge_list.txt \
--make-pgen \
--out input/PMBB_v3_extract/merged/PMBB-Release-2024-3.0_genetic_imputed.ckd_pgs_weights_vars.all_chr
